...
namespace Components.ProceduralGeneration
{
    public abstract class ProceduralGenerationMethod : ScriptableObject
    {
        ...
        //met à jour la view d'une cellule
        protected void SetCellView(Cell cell, string tileName, bool overrideExistingObjects)
        {
            if(cell == null)
                return;
            ...
            {
                UnityEngine.Debug.LogWarning($"Tile template '{tileName}' not found in the database.");
                return;
            }


            // Si la cellule a déjà une view contrôlée, on met à jour sa GridObject et sa position/transform
            if (cell.View != null)
            {
                // Crée un GridObject basé sur le template et attache-le à la View
                var newGridObject = new GridObject(tileTemplate);
                // Si utile, renseigne les coordonnées / données de grille sur l'objet
                newGridObject.SetGridData(cell, Grid);

                // Met à jour les data du cell
                cell.ClearGridObject();
                cell.AddObject(cell.View); // Ajoute la vue existante à la cellule (si nécessaire, vous pouvez adapter ce paramètre)
                newGridObject.SetGridData(cell, Grid);
                cell.View.Initialize(newGridObject);
                var worldPos = cell.GetCenterPosition(Grid.OriginPosition);
                cell.View.MoveTo(worldPos);
                cell.View.ApplyTransform(newGridObject.Rotation, Vector3.one);
            }
            else
            {
                // Fallback : pas de view existante -> utiliser le chemin habituel qui crée la view
                GridGenerator.AddGridObjectToCell(cell, tileTemplate, overrideExistingObjects);
            }
        }
    }
